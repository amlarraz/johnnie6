...
# FUNCIONES BASADAS EN LA URL Y BACKUP DE LA MISMA ---------------------------------------------------------
def __reflash_url__():  # funcion que rehace la URL de base -> http://192.168.1.40:8000/
    global BASE_URL
    BASE_URL = 'http://' + HOST + ':' + PORT + '/'
...
def __write_auto_inf__(ip=None, port=None, rem_status=None):  # funcion que crea el archivo de antes
...
# Hereda QDialog que crea un dialogo de interaccion con el usuario (Para meter la IP y el puerto)
class LoginScreen(QtWidgets.QDialog, Ui_Login_screen):
    ...
    def __init__(self):
        global autologin  # global crea una variable que la modifiques donde la modifiques se vera modificada
        global HOST, PORT

        info = __read_auto_inf__()  # lee los datos del coche ip, port y autologin
        if info == -1:
            HOST = ''
            PORT = ''
            autologin = -1
        ...
        self.setWindowTitle("Log In - SunFounder PiCar-V Client")  # Coloca un nombre a la ventana abierta
...
class RunningScreen(QtWidgets.QDialog, Ui_Running_screen):
    ...
    """
    TIMEOUT = 2000  # Tiempo cada cuanto quieres actualizar el frame

    # Configura las velocidades minimas, maximas y la velocidad para cada boton
    LEVEL1_SPEED = 40
    LEVEL5_SPEED = 100
    ...
    def __init__(self): # Para iniciar la clase
        QtWidgets.QDialog.__init__(self)  # De nuevo, inicia los widgets
        Ui_Running_screen.__init__(self) # lee la interfaz
        self.setupUi(self)  # genera la interfaz

        self.speed_level = 0  # La velocidad inicial es cero
        # when init, no level button has been pressed, self.speed_level = 0
        self.level_btn_show(self.speed_level)  # El boton iluminado es el de la velocidad actual
    ...
    def start_stream(self):
        ...
        self.queryImage = QueryImage(HOST)  # funcion que solicita una imagen al host
        # QTimer mola, es una funcion que cada vez que se le pasa el Timeout que le hayas puesto, ejecuta la
        # funcion que le digas, en este caso reflash frame para recargar el frame que ves
    ...
    def transToPixmap(self):  # Funcion que transforma el dato que proviene del stream (en bits) en un pixmap
        ...
        data = self.queryImage.queryImage() # pide la imagen al servidor
        if not data:
            return None
        pixmap = QPixmap()
        # get pixmap type data from http type data
        pixmap.loadFromData(data) # convierte la imagen
        return pixmap
    ...
    def reflash_frame(self):  # Funcion que recarga el frame,
        ...
        if pixmap:
            ...
            self.label_snapshot.setPixmap(pixmap)  # mete la imaghen en el widget
        else:
            print("frame lost")  # avisa de frame perdido si no la consigue

    def level_btn_show(self, speed_level):  # Funcion que ilumina el boton correcto al cambiar la velocidad
    ...
    def set_speed_level(self, speed):  # set speed to server
        run_speed(speed)  # Pide al servidor que coloque la velocidad correspondiente
    ...
    def keyReleaseEvent(self, event):  # Cuando sueltas, las ruedas vuelven a su estado incial, el coche se para
        # o la camara vuelve al lugar inicial, eso se debe a esto.
    ...
    def on_level1_clicked(self):  # Para cuando se clickea un boton de la velocidad
        # ilumina el boton correspondiente y fija ese nivel de velocidad
...
def run_action(cmd):  # Esta funcion le pide al servidor que haga algo, las posibles opciones estan abajo
